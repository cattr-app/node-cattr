...
class CattrIntervals {
  ...
  async create(interval) {
    ...
    const res = await this.$.post(`api/time-intervals/create`, reqData, { headers: reqData.getHeaders() });
    if (!res.success) {
  ...
  async remove(intervalId) {
    ...
    const res = await this.$.post(`api/time-intervals/remove`, { id: intervalId });
    if (!res.success) {
